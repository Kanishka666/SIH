...
export const UplinkSequence: React.FC<UplinkSequenceProps> = ({
  user,
  mode,
  onComplete,
  onClose
...
void main() {
  ...
  const startSequence = () => {
    ...
    const states = [
      { p: 0, text: 'INITIALIZING LABELLENS' },
      { p: 20, text: mode === 'signup' ? 'ENROLLING COMPLIANCE AUDITOR' : 'VERIFYING CREDENTIALS' },
      { p: 40, text: 'ESTABLISHING SECURE SESSION' },
      { p: 60, text: 'LOADING OCR ENGINE' },
      { p: 80, text: 'LOADING COMPLIANCE MODULES' },
    ...
    const interval = setInterval(() => {
      ...
      if (currentState && statusText !== currentState.text) {
  ...
  const handleCompletion = () => {
    ...
    setTimeout(() => {
      setIsSuccess(true);
      setStatusText(`WELCOME TO LABELLENS, ${user?.name || 'AUDITOR'}`);
  ...
  return (
    <div className="fixed inset-0 z-50 bg-[#161309] text-[#eae2cf] overflow-y-auto min-h-screen flex items-center justify-center p-4 sm:p-6 font-mono selection:bg-[#ffd245] selection:text-black">
      ...
      {/* Main Container - Optimized Compact Proportions */}
        ...
        >
          ...
          <div className="flex justify-between items-end mt-5 sm:mt-7 text-[8px] sm:text-[9px] text-[#d0c6ab]/60 font-mono">
            <div className="flex flex-col gap-0.5">
              <span>IP: [ENCRYPTED-LM-GATEWAY]</span>
              <span>NODE: LL-ALPHA-09</span>
            </div>
            <div className="flex flex-col gap-0.5 text-right">
              <span>
                LATENCY: <span className="text-[#ffd245]/90 font-bold">12ms</span>
              </span>
              <span>{timestamp}</span>
            </div>
          </div>
